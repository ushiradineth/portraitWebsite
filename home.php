...
<body>
    ...
    <div class="bg-img" style="overflow: hidden;">
        ...
        <form id="form1" name="form1" method="post" action="" class="" style="position: absolute; border-radius: 5px; top: 50%; left: 50%; margin-right: -50%; transform: translate(-50%, -50%); height: auto; max-height: 800px; overflow-y: scroll; overflow-x: hidden;"> 
        
		<div class="pad" style="background: white; margin-top: -4px; width:800px; height: auto; padding: 15px 10px; padding-top: 0px; border-radius: 10px 10px 10px 10px; margin-bottom: 10px;">
			<input type="submit" name="profilebtn" id="profilebtn" value="My profile" class="submitbtnbig" style="margin-left: 0%; width: 350px;"/>
			<input type="submit" name="uploadbtn" id="uploadbtn" value="Upload" class="submitbtnbig" style="margin-left: 9%; width: 350px;"/>
        </div>
		<div>
			<?php 
				$con = mysqli_connect("localhost", "root", "", "wadWebsite");
				$sql ="SELECT * FROM `posts` ORDER BY `itemID` desc;";
...
<?php 
    ...
    if(isset($_POST["uploadbtn"])){?>
        <script>document.location.href = "upload.php";</script><?php 
    }
?>